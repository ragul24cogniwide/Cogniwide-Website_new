...
const IndustryTechnologyStack = () => {
  ...
  return (
    <Section background="white" padding="lg" className="bg-gradient-to-br from-gray-50 to-yellow-50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
        {techCategories.map((category, index) => (
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <Card padding="lg" className="h-full hover:shadow-lg transition-all duration-300 hover:shadow-yellow-100 border-l-4 border-l-yellow-400">
              ...
              <div className="flex flex-wrap gap-2">
                {category.technologies.map((tech) => (
                  <span
                    key={tech}
                    className="px-3 py-1 bg-yellow-50 text-sm font-medium text-yellow-700 rounded-full border border-yellow-200 hover:bg-yellow-100 transition-colors"
                  >
                    {tech}
                  </span>
                ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {implementationPhases.map((phase, index) => (
              <div key={phase} className="text-center">
                <div className="w-12 h-12 bg-yellow-500 rounded-full flex items-center justify-center text-black font-bold text-lg mb-4 mx-auto shadow-lg shadow-yellow-500/25">
                  {index + 1}
                </div>
